...
in which distributed nodes (or workers, which we call EvoWorkers) asynchronously take a sample of the population pool, apply some evolutionary search procedure 
...
other volunteer computing platforms; very early papers lie the one by
...
insfrastructure; using higher-level cloud tools like map-reduce is
...
parameters needed to obtain solutionos efficiently. There are many
approaches to optimal EA parametrization since the first papers;
aproaches were initially reviewed in \cite{de2007parameter}, with more
recent reviews published by Karafotias et al. \cite{6747993} and Aleti
and Moser \cite{Aleti2016}. However, with the popularity and afordability of computing
...
randcomly setting the parameters of each subpopulation when the island
...
distributed evoltuionary algorithms was published in \cite{LNCS86720702}, where we proved that in effect our
...
parts, which are also shown in Figure
\ref{fig:evospace}:\begin{itemize}
\item The EvoSpace {\em pool}, with the evolving population and is
  decoupled from,
\item the workers, which we call EvoWorkers, and are the ones actually
...
In a basic EvoSpace configuration, EvoWorkers request a small random sample of the
population, using it as the initial seed for a local EA executed
on the clients, containers or cloud instances; this EA returns the result to the pool. The experimental platform used in this work is
...
aforementioned Figure shows.  In general, the number of function
...
\begin{figure*}[t]
    ...
    \subfigure  [Heterogeneous]
    {
      ...
      \caption{30 runs on the Griewank function. Box-plot of the
...
\begin{figure*}[t]
    ...
    \caption{30 runs on the Schaffer function. Box-plot of the number
...
when it might not find the solution using the minimun number of
...
initially by the work of Fukunaga et al; it also shows that the random
...
while in the case of Fukunaga et al. a large number of islands was
